...
import { fetchDashboard, fetchAccounts, fetchBudgets, DEFAULT_USER_ID } from "@/src/lib/api"
...
export default function DashboardPage() {
  ...
  const profileName = onboarding?.profile?.name || "there"
  const onboardingIncome = onboarding?.income?.hasNoIncome
    ? 0
    : Math.max(onboarding?.income?.amount || 0, 0)
  const onboardingBillsTotal = useMemo(() => {
    if (!onboarding?.bills?.length) return 0
    return onboarding.bills.reduce((sum, bill) => sum + (Number(bill.amount) || 0), 0)
  }, [onboarding?.bills])
  const onboardingLeftover = Math.max(onboardingIncome - onboardingBillsTotal, 0)
    ...
    <main className="min-h-screen bg-slate-50 p-4 text-slate-900">
      <div className="mx-auto flex max-w-md flex-col gap-6 pt-8">
        <Card className="border-emerald-200 bg-emerald-50">
          <CardContent className="p-4">
            <p className="text-sm text-emerald-700">Welcome back, {profileName}.</p>
            {onboarding && (
              <p className="text-base font-semibold text-emerald-900">
                Monthly income: RM {onboardingIncome.toFixed(2)} · Bills: RM{" "}
                {onboardingBillsTotal.toFixed(2)} · Leftover: RM {onboardingLeftover.toFixed(2)}
              </p>
            )}
          </CardContent>
        </Card>
